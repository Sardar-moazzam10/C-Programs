...
function Startingwithlettera(words) {
...
function arrayscore() {
    let array_scores = [10, 5, 20, 20, 4, 5, 2, 25, 1];
...
function findDuplicates(arr) {
    const duplicates = arr.filter((itemm, indexx) => arr.indexOf(itemm) !== indexx);
...
function convertToFahrenheit(tempsFahrenheit) {
    const tempsCelsius = tempsFahrenheit.map((temp) => (temp - 32) * (5 / 9));
...
console.log(menu);
function calculatoR(numm1, numm2, operator) {
    if (operator === "+") {
